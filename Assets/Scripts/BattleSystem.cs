...
public class BattleSystem : MonoBehaviour
{
	...
	private void Start()
    {
	    //iceWizardDead.SetActive(false);
		//fireWizardDead.SetActive(false);
		//lighteningWizardDead.SetActive(false);
    ...
    private IEnumerator SetupBattle()
    {
	    ...
	    else if (!isFireWizardDead)
	    {
		    state = BattleState.FIRETURN;
		    FireTurn();
		} else if (!isLighteningWizardDead) {
    ...
    private IEnumerator EnemyTurn()
    {
        ...
        if (!isIceWizardDead)
        {
			...
			{
				// Spawn attack sound effect
				Instantiate(fireAttackSound, iceWizardInstance.transform.position, Quaternion.identity);
			}
			if (enemyInstance.CompareTag("Lightening_Enemy")) {
				// Spawn attack sound effect
				Instantiate(lighteningAttackSound, iceWizardInstance.transform.position, Quaternion.identity);
			}
        ...
        if (isIceWizardDead && isFireWizardDead && isLighteningWizardDead) {
	        // End the battle
	        state = BattleState.LOST;
	        EndBattle();
        } 
        else if (!isIceWizardDead)
        {
    ...
    IEnumerator FireAttack() {
		...
		yield return new WaitForSeconds(2.0f);
		// Damage the enemy
		bool isEnemyDead = enemyUnit.EnemyTakeDamage(fireUnit.fireDamage);
    ...
    private IEnumerator IceHeal()
    {
		...
		Instantiate(healSound, iceWizardInstance.transform.position, Quaternion.identity);
